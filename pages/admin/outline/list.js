  ...
  initTreeData(list, item) {
    if (!item) {
      ...
      list.filter(c => c.parent_id == 0).map(c => {
        let obj = { key: c.id, id: c.id, parent_id: c.parent_id, section_name: c.section_name, is_free: c.is_free, children: [] }
        treeData.push(obj)
      ...
      list.filter(c => c.parent_id == item.id).map(c => {
        let obj = { key: c.id, id: c.id, parent_id: c.parent_id, section_name: c.section_name, is_free: c.is_free }
    ...
    return (<div style={{ minHeight: 800, backgroundColor: 'white', marginRight: '20px' }}>
      ...
      <Row>
        <Col className={css.mg} span={24}>
          {/* 2.0 搜索区 */}
          <Button.Group>
            <Link href={{ pathname: '/admin/outline/add' }}>
              <Button>
                <Icon type="plus" />新增
            </Button>
            </Link>
