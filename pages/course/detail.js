            ...
            <div className={css.article_banner}>
                ...
                <div className={css.banner_info}>
                    <div className={css.banner_left}>
                        <p>
                            {
                                this.props.blist && this.props.blist.map((item, i) => (
                                    <span key={i}>{this.props.blist.length - 1 == i ? item.title : item.title + ' \\ '}</span>
                                ))
                            }
                        </p>
                        <p className={css.tit}>{this.props.courseInfo.title}</p>
                        <p className={css.pic}><span className={css.new_pic}>特惠价格￥{this.props.courseInfo.sell_price}</span>
                            <span className={css.old_pic}>原价￥{this.props.courseInfo.market_price}</span></p>
                        <p className={css.info}>
                            <a onClick={this.intoShopCar.bind(this)} href="#">加入购物车</a>
                            <span><em>难度等级</em>{this.props.courseInfo.lesson_level}</span>
                            <span><em>课程时长</em>{this.props.courseInfo.lesson_time}</span>
                            <span><em>评分</em>{this.props.courseInfo.lesson_star}分</span>
                            <span><em>授课模式</em>{this.props.courseInfo.leson_type}</span>
                        </p>
                    </div>
                            ...
                            <Tabs defaultActiveKey="1">
                                <TabPane tab={<span><Icon type="file-text" />课程详情</span>} key="1">
                                    {/*
                                        ...
                                        可以实现html代码的渲染
                                    */}
                                    <div className={css.tabp} dangerouslySetInnerHTML={{ __html: this.props.courseInfo.content }}>
                                ...
                                <TabPane tab={<span><Icon type="question-circle" />常见问题</span>} key="4">
                                    <div className={css.tabp} dangerouslySetInnerHTML={{ __html: this.props.courseInfo.common_question }}>
                                    </div>
                                </TabPane>
                            </Tabs>
                        </div>
